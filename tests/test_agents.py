...
def test_agent_workflow():
    ...
    assert df_cleaned['Income'].isna().sum() == 0, "Income missing values should be imputed"
    ...
    try:
        figures, text = supervisor.handle_query(df_cleaned, "What is the average income?", api_key="dummy_key")
        print(f"\nResult: {text}")
