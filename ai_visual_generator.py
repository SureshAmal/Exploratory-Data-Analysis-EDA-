...
def generate_targeted_visuals(df, question):
    ...
    is_about_sales = 'sales' in question_lower or 'revenue' in question_lower or 'amount' in question_lower
    is_about_time = 'time' in question_lower or 'trend' in question_lower or 'over' in question_lower or 'date' in question_lower
    is_about_product = 'product' in question_lower
    is_about_region = 'region' in question_lower or 'location' in question_lower
    is_about_comparison = 'compare' in question_lower or 'versus' in question_lower or 'vs' in question_lower
    is_about_distribution = 'distribution' in question_lower or 'spread' in question_lower
    ...
    for col in df.columns:
        ...
        if 'customer' in col_lower and is_about_customer:
            customer_col = col
        if ('total' in col_lower or 'amount' in col_lower or 'sales' in col_lower or 'revenue' in col_lower) and is_about_sales:
            sales_col = col
    ...
    if is_about_customer and customer_col and sales_col:
        ...
        customer_sales = df.groupby(customer_col)[sales_col].sum().sort_values(ascending=False).head(15)
        fig = px.bar(
        ...
        if len(df) > 0:
            customer_count = df.groupby(customer_col).size().sort_values(ascending=False).head(15)
            fig_count = px.bar(
    ...
    # PRODUCT-BASED ANALYSIS
        ...
        # Sales by product
        product_sales = df.groupby(product_col)[sales_col].sum().sort_values(ascending=False).head(15)
        fig = px.bar(
...
Be concise and only mention relevant insights."""
            ...
            if ai_response.startswith(('Error:', 'API error:', 'HTTP error', 'Unable to extract')):
