...
def render_sales(df_sales: Optional[pd.DataFrame], parent: st.delta_generator.DeltaGenerator | None = None):
    ...
    # If no sales dataframe provided, show a helpful message and exit early
    if df_sales is None:
        s.info("Sales dashboard disabled — dataset is missing Date/Customer/Total_Amount columns.")
        return

    s.markdown("## Sales Analytics Dashboard")
    s.info("Sales dataset detected - Specialized visualizations enabled")

    sales_tab1, sales_tab2, sales_tab3, sales_tab4 = s.tabs(
        [
            "Sales Over Time",
            "Sales by Customer",
            "Product Analysis",
    ...
    with sales_tab1:
        ...
        with col1:
            ...
            s.plotly_chart(fig_daily, width="stretch")
            ...
            s.plotly_chart(fig_monthly, width="stretch")
        ...
        with col2:
            ...
            s.plotly_chart(fig_cumulative, width="stretch")
            ...
            s.plotly_chart(fig_yearly, width="stretch")

    with sales_tab2:
        s.markdown("### Sales by Customer Analysis")
        customer_stats = (
            df_sales.groupby("Customer").agg({"Total_Amount": ["sum", "mean", "count"]}).round(2)
        )
        ...
        col1, col2 = s.columns(2)
        with col1:
            top_n = s.slider("Number of top customers to display", 5, 20, 10, key="top_customers_slider")
            ...
            s.plotly_chart(fig_customers, width="stretch")

        with col2:
            fig_pie = px.pie(top_customers, values="Total_Sales", names="Customer", title=f"Revenue Distribution - Top {top_n} Customers", hole=0.4, template=BRAND_TEMPLATE)
            s.plotly_chart(fig_pie, width="stretch")

        s.markdown("<br>", unsafe_allow_html=True)
        s.markdown("### Customer Statistics Table")
        s.dataframe(
            customer_stats.head(15).style.format({"Total_Sales": "${:,.2f}", "Avg_Transaction": "${:,.2f}", "Transaction_Count": "{:.0f}"}),
            width="stretch",
        )
    ...
    with sales_tab3:
        ...
        if "Product" in df_sales.columns:
            ...
            with col1:
                ...
                s.plotly_chart(fig_products, width="stretch")
                ...
                if "Quantity" in df_sales.columns:
                    ...
                    fig_qty = px.bar(product_qty, x="Product", y="Quantity", title="Units Sold by Product", labels={"Quantity": "Units Sold", "Product": "Product"}, color="Quantity", color_continuous_scale="Greens", template=BRAND_TEMPLATE)
                    fig_qty.update_layout(xaxis_tickangle=-45)
                    s.plotly_chart(fig_qty, width="stretch")

            with col2:
                fig_product_pie = px.pie(product_sales.head(10), values="Total_Amount", names="Product", title="Top 10 Products Revenue Share", hole=0.3, template=BRAND_TEMPLATE)
                s.plotly_chart(fig_product_pie, width="stretch")
                ...
                fig_product_trend = px.line(product_timeline_top, x="Month_Year", y="Total_Amount", color="Product", title="Top 5 Products Sales Trend", labels={"Total_Amount": "Sales ($)", "Month_Year": "Month"}, template=BRAND_TEMPLATE)
                s.plotly_chart(fig_product_trend, width="stretch")
        else:
    ...
    with sales_tab4:
        ...
        with metric_col1:
            s.metric(label="Total Revenue", value=f"${total_revenue:,.2f}")
            s.metric(label="Total Transactions", value=f"{total_transactions:,}")

        with metric_col2:
            s.metric(label="Avg Transaction", value=f"${avg_transaction:,.2f}")
            s.metric(label="Total Customers", value=f"{total_customers:,}")

        with metric_col3:
            s.metric(label="Avg per Customer", value=f"${avg_per_customer:,.2f}")
            if "Product" in df_sales.columns:
                total_products = df_sales["Product"].nunique()
                s.metric(label="Total Products", value=f"{total_products:,}")

        with metric_col4:
            date_range_days = (df_sales["Date"].max() - df_sales["Date"].min()).days
            s.metric(label="Date Range (Days)", value=f"{date_range_days:,}")
            daily_avg = total_revenue / max(date_range_days, 1)
            s.metric(label="Avg Daily Sales", value=f"${daily_avg:,.2f}")

        s.markdown("---")
        col1, col2 = s.columns(2)
        with col1:
            s.markdown("### Top Performers")
            daily_sales = df_sales.groupby("Date")["Total_Amount"].sum().reset_index()
            best_day = daily_sales.loc[daily_sales["Total_Amount"].idxmax()]
            s.write(f"**Best Sales Day:** {best_day['Date'].strftime('%Y-%m-%d')} (${best_day['Total_Amount']:,.2f})")

            customer_stats = df_sales.groupby("Customer").agg({"Total_Amount": ["sum", "mean", "count"]}).round(2)
            customer_stats.columns = ["Total_Sales", "Avg_Transaction", "Transaction_Count"]
            customer_stats = customer_stats.reset_index().sort_values("Total_Sales", ascending=False)
            best_customer = customer_stats.iloc[0]
            s.write(f"**Top Customer:** {best_customer['Customer']} (${best_customer['Total_Sales']:,.2f})")

            if "Product" in df_sales.columns:
                product_sales = df_sales.groupby("Product")["Total_Amount"].sum().reset_index()
                product_sales = product_sales.sort_values("Total_Amount", ascending=False)
                best_product = product_sales.iloc[0]
                s.write(f"**Top Product:** {best_product['Product']} (${best_product['Total_Amount']:,.2f})")

        with col2:
            s.markdown("### Distribution Insights")
            if "Region" in df_sales.columns:
                ...
                fig_region = px.bar(region_sales, x="Region", y="Total_Amount", title="Sales by Region", color="Total_Amount", color_continuous_scale="Blues", template=BRAND_TEMPLATE)
                s.plotly_chart(fig_region, width="stretch")
            else:
                s.info("Regional data not available")
